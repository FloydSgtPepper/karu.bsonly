...
public class TestMemoryReader
{
  ...
  public void InitializationTest1()
  {
    ...
    bson_doc = Utils.HexConverter.HexStringToByteArray(bson_str);
    serializer = new MemoryReader(bson_doc, 64 * 1024 * 1024, false); // must not throw
  ...
  public void InitializationTestWrongDocSize()
  {
    ...
    var bson_doc = Utils.HexConverter.HexStringToByteArray(bson_str);
    // ctor must not throw
    Assert.ThrowsExactly<ArgumentException>(() => new MemoryReader(bson_doc, 64 * 1024 * 1024, false));
  }
  ...
  public void TwoElementsTest()
  {
    ...
    serializer = new MemoryReader(bson_doc, 64 * 1024, false);
  ...
  public void OutOfOrderDeserialization()
  {
    ...
    serializer = new MemoryReader(bson_doc, max_doc_size: 64 * 1024 * 1024, out_of_order_evaluation: false);
    Assert.IsFalse(serializer.HasEntry("StrProperty"u8, BsonConstants.BSON_TYPE_DOUBLE));

    serializer = new MemoryReader(bson_doc, max_doc_size: 64 * 1024 * 1024, out_of_order_evaluation: true);
  ...
  public void ReadDocument()
  {
    ...
    var deserializer = new MemoryReader(bson_doc, int.MaxValue, out_of_order_evaluation: true);
    ...
    var tc = new TestClass();
    doc_deserializer.ReadDocument(tc, _context);
