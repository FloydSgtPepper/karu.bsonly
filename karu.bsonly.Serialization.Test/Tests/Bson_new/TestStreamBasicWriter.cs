...
[TestClass]
public class TestStreamBasicWriter
{
  [TestInitialize]
  ...
  {
    var expected_bson = Regex.Replace(@"0x2a00000000000000", "\\s+", "");

    var doc_stream = new MemoryStream();

    var serializer = new StreamBasicWriter(doc_stream);
    serializer.WriteLong(42L);
    var actual = doc_stream.ToArray();
  ...
  {
    var expected_bson = Regex.Replace(@"0x11000000 74686973206973206120737472696e6700", "\\s+", "");

    var doc_stream = new MemoryStream();

    var serializer = new StreamBasicWriter(doc_stream);
    serializer.WriteString("this is a string"u8);
    var actual = doc_stream.ToArray();
  ...
  //   var serializer = new karu.bsonly.Serialization.StreamWriter();
