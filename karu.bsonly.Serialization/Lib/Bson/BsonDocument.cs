...
public class BsonDocument : IBsonDocument, IDisposable
{
  // implementation:
  ...
  /// <exception cref="ArgumentException"></exception>
  public BsonDocument(BsonDocument parent)
  {
    _parent = parent;
    _doc = parent._doc;
  ...
  public IBsonDocument NextElement()
  {
  ...
  public (ReadOnlyMemory<byte> key_string, byte type) NextEntry()
  {
    ...
    if (_iterator._state == IterState.AtElement)
      return new(_iterator._element_key, _iterator._element_type);
    ...
    Finish();
    return new(ReadOnlyMemory<byte>.Empty, BsonConstants.BSON_TYPE_EOD);
  }
